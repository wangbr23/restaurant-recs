...
class RestaurantModel():
    ...
    def train_model(self):
        x, y = self.build_training_tensors()
    ...
    def evaluate_model(self):
        with torch.no_grad():
            predictions = self.model(self.x_tensor)

        predicted_labels = (predictions >= 0.5).float()
        correct = (predicted_labels == self.y_tensor).sum().item()
        total = len(TRAINING_ROWS)
        accuracy = correct / total

        return accuracy, predictions, self.y_tensor
